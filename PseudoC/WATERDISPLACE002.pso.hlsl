//
// Generated by Microsoft (R) D3DX9 Shader Compiler 9.08.299.0000
//
//   vsa shaderdump19/WATERDISPLACE002.pso
//    /Fcshaderdump19/WATERDISPLACE002.pso.dis
//
//
// Parameters:
//
//   sampler2D HeightMap;
//   float3 WadingVars;
//
//
// Registers:
//
//   Name         Reg   Size
//   ------------ ----- ----
//   WadingVars   const_5       1
//   HeightMap    texture_0       1
//

    const float4 const_0 = {0, -(1.0 / 256), 4, -0.5};
    const float4 const_1 = {-(1.0 / 256), 0, 0, 0};
    float2 texcoord_0 : TEXCOORD0;
    sampler2D HeightMap;
    r3.xy = IN.texcoord_0.xy + const_0.xy;
    r2.xy = IN.texcoord_0.xy - const_0.xy;
    r1.xy = IN.texcoord_0.xy + const_1.xy;
    r0.xy = IN.texcoord_0.xy - const_1.xy;
    r3.xyzw = tex2D(HeightMap, r3.xy);
    r4.xyzw = tex2D(HeightMap, r2.xy);
    r2.xyzw = tex2D(HeightMap, r1.xy);
    r1.xyzw = tex2D(HeightMap, r0.xy);
    r0.xyzw = tex2D(HeightMap, IN.texcoord_0.xy);
    r1.w = r3.x + r4.x;
    r1.w = r2.x + r1.w;
    r1.w = r1.x + r1.w;
    r1.w = (r0.x * -4) + r1.w;
    r0.xyzw = r0.xyzw - 0.5;
    r0.y = (WadingVars.x * r1.w) + r0.y;
    r0.x = (WadingVars.y * r0.y) + r0.x;
    r1.x = -0.5;
    r0.xyzw = (WadingVars.z * r0.xyzw) - r1.x;
    OUT.color_0.rgba = r0.xyzw;

// approximately 19 instruction slots used (5 texture, 14 arithmetic)
