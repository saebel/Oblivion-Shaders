//
// Generated by Microsoft (R) D3DX9 Shader Compiler 9.08.299.0000
//
//   psa shaderdump19/WATER007.pso /Fcshaderdump19/WATER007.pso.dis
//
//
// Parameters:
//
//   float2 BlendRadius;
//   float4 DeepColor;
//   sampler2D DepthMap;
//   sampler2D DetailMap;
//   sampler2D DisplacementMap;
//   float4 EyePos;
//   float4 FogColor;
//   float4 FogParam;
//   float FresnelRI;
//   sampler2D NormalMap;
//   float4 ReflectionColor;
//   sampler2D ReflectionMap;
//   float2 Scroll;
//   float4 ShallowColor;
//   float4 SunColor;
//   float4 SunDir;
//   float4 VarAmounts;
//
//
// Registers:
//
//   Name            Reg   Size
//   --------------- ----- ----
//   Scroll          const_0       1
//   EyePos          const_1       1
//   SunDir          const_2       1
//   SunColor        const_3       1
//   ShallowColor    const_5       1
//   DeepColor       const_6       1
//   ReflectionColor const_7       1
//   VarAmounts      const_8       1
//   FogParam        const_9       1
//   FogColor        const_10      1
//   FresnelRI       const_11      1
//   BlendRadius     const_12      1
//   ReflectionMap   texture_0       1
//   NormalMap       texture_1       1
//   DetailMap       texture_2       1
//   DepthMap        texture_3       1
//   DisplacementMap texture_4       1
//

    const float4 const_4 = {-0.5, 0.1, 0.0002, 0.25};
    const float4 const_13 = {2496, 4, -0.2, -0.55};
    const float4 const_14 = {(1.0 / 0.35), 1, 0, 0};
    const float4 const_15 = {2, -1, 0, -(1.0 / 8192)};
    float3 texcoord_0 : TEXCOORD0_centroid;
    float3 texcoord_1 : TEXCOORD1_centroid;
    float4 texcoord_2 : TEXCOORD2_centroid;
    float4 texcoord_3 : TEXCOORD3_centroid;
    float4 texcoord_4 : TEXCOORD4_centroid;
    float4 texcoord_5 : TEXCOORD5_centroid;
    float4 texcoord_6 : TEXCOORD6;
    float4 texcoord_7 : TEXCOORD7;
    sampler2D ReflectionMap;
    sampler2D NormalMap;
    sampler2D DetailMap;
    sampler2D DepthMap;
    sampler2D DisplacementMap;
    r0.xy = EyePos - IN.texcoord_1;
    r0.w = dot(r0.xy, r0.xy) + const_15.z;
    r0.w = 1.0 / sqrt(r0.w);
    r4.w = 1.0 / r0.w;
    r2.w = saturate((r4.w * const_15.w) - const_15.y);
    r0.xy = IN.texcoord_6 + const_4.x;
    r0.w = dot(r0.xy, r0.xy) + const_15.z;
    r0.w = 1.0 / sqrt(r0.w);
    r3.w = 1.0 / r0.w;
    r0.w = r3.w + r3.w;
    r1.w = 1.0 / BlendRadius.x;
    r1.w = r0.w * r1.w;
    r0.w = saturate(max(const_4.y, r1.w));
    r5.w = -(r0.w + const_15.y);
    r6.w = r2.w * r2.w;
    r4.x = IN.texcoord_7.z + Scroll.x;
    r4.y = IN.texcoord_7.w + Scroll.y;
    r1 = tex2D(NormalMap, r4);
    r0 = tex2D(DisplacementMap, IN.texcoord_6);
    r2.xyz = (const_15.x * r1) + const_15.y;
    r2.xy = r6.w * r2;
    r1.w = r5.w * BlendRadius.y;
    r1.xyz = (const_15.x * r0) + const_15.y;
    r0.w = saturate(r4.w * const_4.z);
    r0.xyz = lerp(r1, r2, r1.w);
    r0.w = (r0.w * const_13.x) + const_13.y;
    r3.xyz = normalize(r0);
    r1.xy = (r0.w * r3) + IN.texcoord_0;
    r1.z = IN.texcoord_0.z;
    r1.w = -const_15.y;
    r0.x = dot(IN.texcoord_2, r1);
    r0.y = dot(IN.texcoord_3, r1);
    r0.z = dot(IN.texcoord_4, r1);
    r0.w = dot(IN.texcoord_5, r1);
    r5.xy = const_15;
    r3.w = (r5.x * -r3.w) + BlendRadius.x;
    r1.xy = (const_4.y * r3) + r4;
    r6.w = r2.w * VarAmounts.w;
    r4.x = IN.texcoord_6.z;
    r4.y = IN.texcoord_6.w;
    r2 = tex2Dproj(ReflectionMap, r0);			// partial precision
    r1 = tex2D(DetailMap, r1);
    r0 = tex2D(DepthMap, r4);
    r4.xyz = EyePos - IN.texcoord_1;
    r5.x = dot(r4, r4);	// normalize + length
    r0.w = 1.0 / sqrt(r5.x);
    r4.xyz = r4 * r0.w;
    r0.w = 1.0 / r0.w;
    r5.x = saturate(dot(r4, r3));
    r1.w = -(r5.x + const_15.y);
    r2.w = r1.w * r1.w;
    r2.w = r2.w * r2.w;
    r4.w = r1.w * r2.w;
    r2.w = -(r5.y + FresnelRI.x);
    r6.x = dot(-r4, r3);
    r1.w = r6.x + r6.x;
    r9.w = (r2.w * r4.w) + FresnelRI.x;
    r3.xyz = (-r1.w * r3) - r4;
    r4.w = max(VarAmounts.z, r9.w);
    r3.x = saturate(dot(r3, SunDir));
    r7.w = pow(abs(r3.x), VarAmounts.x);
    r2.w = saturate(SunDir.w);
    r0.w = FogParam.x - r0.w;
    r1.w = 1.0 / FogParam.y;
    r0.w = saturate(r0.w * r1.w);
    r1.w = -(r0.w + const_15.y);
    r5.w = const_4.w - r4.w;
    r0.w = -(r0.x + const_15.y);
    r5.w = (r0.w * r5.w) + r4.w;
    r0.w = r0.x + const_15.y;
    r2.xyz = r2 - ReflectionColor;
    r4.w = (r0.w >= 0.0 ? r5.w : r4.w);
    r0.z = VarAmounts.y;
    r2.xyz = (r0.z * r2) + ReflectionColor;			// partial precision
    r0.w = r0.x + const_13.z;
    r3.xyz = DeepColor;
    r3.xyz = ShallowColor - r3;
    r5.w = (r0.w * -const_14.x) + const_14.y;
    r4.xyz = (r5.x * r3) + DeepColor;			// partial precision
    r8.w = r5.w * r5.w;
    r3.xyz = lerp(r2, r4, r9.w);
    r5.w = (r5.w * -r8.w) - const_15.y;
    r2.xyz = r7.w * SunColor;
    r5.w = r4.w * r5.w;
    r3.xyz = (r2.w * r2) + r3;
    r2.w = r0.x + const_13.w;
    r2.xyz = lerp(r1, r3, r6.w);
    r2.w = (r2.w >= 0.0 ? r5.w : r4.w);
    r1.xyz = FogColor - r2;
    r0.w = (r0.w >= 0.0 ? const_15.z : r2.w);
    r1.xyz = (r1.w * r1) + r2;
    r1.w = (r3.w >= 0.0 ? const_15.z : r0.w);
    r0 = (r0.x <= 0.0 ? r1 : const_15.z);
    OUT.color_0 = r0;

// approximately 101 instruction slots used (5 texture, 96 arithmetic)
