//
// Generated by Microsoft (R) D3DX9 Shader Compiler 9.08.299.0000
//
//   vsa shaderdump19/SLS2015.pso /Fcshaderdump19/SLS2015.pso.dis
//
//
// Parameters:
//
//   float4 AmbientColor;
//   sampler2D BaseMap;
//   sampler2D NormalMap;
//   float4 PSLightColor[4];
//   float4 Toggles;
//
//
// Registers:
//
//   Name         Reg   Size
//   ------------ ----- ----
//   AmbientColor AmbientColor       1
//   PSLightColor PSLightColor       1
//   Toggles      Toggles       1
//   BaseMap      BaseMap       1
//   NormalMap    NormalMap       1
//

    const_0 = {-0.5, 0.200000003, 0.5, 0};
    texcoord input_0.xy;			// partial precision
    texcoord input_1.xyz;			// partial precision			// centroid
    texcoord input_3.xyz;			// partial precision			// centroid
    color input_0.xyz;
    color input_1;
    sampler BaseMap;
    sampler NormalMap;
    r1 = NormalMap[texcoord_0];			// partial precision
    r0 = BaseMap[texcoord_0];			// partial precision
    r2.xyz = norm(texcoord_3);			// partial precision
    r1.xyz = r1 - const_0.x;
    r3.xyz = r1 - r1;			// partial precision
    r1.xyz = norm(r3);			// partial precision
    r3.x = sat((r1.x * r2.x) + (r1.y * r2.y) + (r1.z * r2.z));			// partial precision
    pow r2.w, r3.x, Toggles.z			// partial precision
    r3.x = (r1.x * texcoord_1.x) + (r1.y * texcoord_1.y) + (r1.z * texcoord_1.z);			// partial precision
    r3.w = r1.w * r2.w;			// partial precision
    r2.w = r3.x - const_0.z;			// partial precision
    r1.w = (r2.w >= const_0.w ? r2.w : const_0.w);			// partial precision
    r2.w = r3.w * r1.w;			// partial precision
    r1.w = const_0.y - r3.x;			// partial precision
    r1.w = (r1.w >= 0.0 ? r3.w : r2.w);			// partial precision
    r2.xyz = sat(r1.w * PSLightColor);			// partial precision
    r1.xyz = r0 * input_0;			// partial precision
    r1.w = sat(r3.x);			// partial precision
    r1.xyz = (Toggles.x <= 0.0 ? r1 : r0);			// partial precision
    r0.xyz = PSLightColor;
    r3.xyz = (r1.w * r0) + AmbientColor;			// partial precision
    r0.xyz = (r3 >= const_0.w ? r3 : const_0.w);			// partial precision
    r0.xyz = (r1 * r0) + r2;			// partial precision
    r1.xyz = input_1.w * (input_1 - r0) + r0;			// partial precision
    r0.w = r0.w * AmbientColor.w;			// partial precision
    r0.xyz = (Toggles.y <= 0.0 ? r1 : r0);			// partial precision
    rendertarget_0 = r0;			// partial precision

// approximately 34 instruction slots used (2 texture, 32 arithmetic)
