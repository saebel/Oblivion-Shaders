//
// Generated by Microsoft (R) D3DX9 Shader Compiler 9.08.299.0000
//
//   vsa shaderdump19/STLEAF003.vso /Fcshaderdump19/STLEAF003.vso.dis
//
//
// Parameters:

float4 AmbientColor;
float4 BillboardRight;
float4 BillboardUp;
float4 DiffColor;
float4 DiffColorPt;
float3 FogColor;
float4 FogParam;
float4 LeafBase[48];
float4 LeafLighting;
float4 LightPos;
float4 LightVector;
row_major float4x4 ModelViewProj;
float4 RockParams;
float4 RustleParams;
float SunDimmer;
float4 WindMatrices[16];

// Structures:

struct VS_INPUT {
    float4 position : POSITION;
    float3 normal : NORMAL;
    float4 texcoord_0 : TEXCOORD0;
    float4 blendindices : BLENDINDICES;
};

struct VS_OUTPUT {
    float4 position : POSITION;
    float2 texcoord_0 : TEXCOORD0;			// partial precision
    float4 ilum : TEXCOORD1;			// partial precision
    float4 fog : TEXCOORD2;			// partial precision
};

// Code:

VS_OUTPUT main(VS_INPUT IN) {
    VS_OUTPUT OUT;

#define	PI	3.14159274

    const float4 const_4 = {PI * 2, -PI, -2.52398507e-007, 2.47609005e-005};
    const float4 const_82 = {(1.0 / (PI * 2)), 0.25, 0.5, 0};
    const float4 const_83 = {(1.0 / 48), 0.499999553, 0.25, -0.00138883968};
    const float const_84 = (1.0 / 24);
    float4 IN_position, mtx;
    float4 r0, r1, r2, r4, r5;
    float2 r3;
    float offset;
    float irock, irstl;
    float ir0, ir1, ir2, is0, is1, is2;
    float2 rr0, rr1;
    float2 irm, ism;

    irock = (IN.blendindices.z * const_83.x) + RockParams.y;
    irstl = (IN.blendindices.z * const_83.x) + RustleParams.y;

    irock = (irock * const_83.y) + const_83.z;
    irstl = (irstl * const_83.y) + const_83.z;

    irock = pow(2.0, irock);	// partial precision
    irstl = pow(2.0, irstl);	// partial precision

    ir1 = (irock * const_4.x) + const_4.y;
    is0 = (irstl * const_4.x) + const_4.y;
    ir0 = (ir1 * ir1 * const_4.z) + const_4.w;
    is1 = (is0 * is0 * const_4.z) + const_4.w;

    ir2 = (ir1 * ir0) + const_83.w;
    is1 = (is0 * is1) + const_83.w;
    ir2 = (ir1 * ir2) + const_84;
    is1 = (is0 * is1) + const_84;
    ir2 = (ir1 * ir2) + -0.5;
    is1 = (is0 * is1) + -0.5;
    ir2 = (ir1 * ir2) + 1;
    is0 = (is0 * is1) + 1;
    ir2 = ir2 * RockParams.z * RockParams.x;
    is0 = is0 * RustleParams.z * RustleParams.x;

    rr0 = frac((ir2 * const_82.x) + const_82.yzzw);
    rr1 = frac((is0 * const_82.x) + const_82.yzzw);
    rr0 = (const_4.x * rr0) + const_4.y;
    rr1 = (const_4.x * rr1) + const_4.y;
    rr0 = rr0 * rr0;
    rr1 = rr1 * rr1;

    irm = (const_4.z * rr0) + const_4.w;
    ism = (const_4.z * rr1) + const_4.w;
    irm = (irm * rr0) + const_83.w;
    ism = (ism * rr1) + const_83.w;
    irm = (irm * rr0) + const_84;
    ism = (ism * rr1) + const_84;
    irm = (irm * rr0) + -0.5;
    ism = (ism * rr1) + -0.5;

    r1.xw = (rr0.yyxx * irm.yyxx) + 1;
    r2.xw = (rr1.yyxx * ism.yyxx) + 1;
    r1.y = -r1.w;
    r2.y = -r2.w;

    /* ------------------------------------------------------------------------- */
    offset = IN.blendindices.z;
    r0 = IN.blendindices.w * LeafBase[offset];

    r5.x = dot(r1.xyw, r0);
    r4.x = dot(r1.wxw, r0);

    mtx.xy = ism;
    mtx.xw = r2.xw;
    mtx.y = r2.y;
    mtx.z = 0;

    r2.x = dot(mtx, BillboardRight);
    r2.y = dot(mtx.wxzw, BillboardRight);
    r2.zw = BillboardRight;

    r1.x = dot(mtx, BillboardUp);
    r1.y = dot(mtx.wxzw, BillboardUp);
    r1.zw = BillboardUp;

    r1 = (r4.x * r1) + (r5.x * r2);

    IN_position = r1 + IN.position;

    /* +term: wind ------------------------------------------------------------- */
    float4 InstanceWind, InstanceProj;
    offset = IN.blendindices.y;

    InstanceWind.x = dot(WindMatrices[0 + offset], IN_position);
    InstanceWind.y = dot(WindMatrices[1 + offset], IN_position);
    InstanceWind.z = dot(WindMatrices[2 + offset], IN_position);
    InstanceWind.w = dot(WindMatrices[3 + offset], IN_position);

    InstanceWind =                      InstanceWind  - IN_position;
    InstanceWind = (IN.blendindices.x * InstanceWind) + IN_position;

    InstanceProj.x = dot(ModelViewProj[0], InstanceWind);
    InstanceProj.y = dot(ModelViewProj[1], InstanceWind);
    InstanceProj.z = dot(ModelViewProj[2], InstanceWind);
    InstanceProj.w = dot(ModelViewProj[3], InstanceWind);

    OUT.position = InstanceProj;

    /* +term: ??? -------------------------------------------------------------- */
    float3 Surface;

    Surface.xyz = normalize(r0);
    Surface.xyz = (LeafLighting.y * Surface) + IN.normal;
    Surface.xyz = normalize(Surface);

    /* +term: directional point light (torch) ---------------------------------- */
    float3 pntvec;
    float pntdst;
    float4 pntcol;

    pntvec = LightPos - InstanceWind;
    pntdst = length(pntvec);
    pntvec = normalize(pntvec);

    pntdst = min(max(pntdst / LightPos.w, 0), 1);
    pntdst = (pntdst * -pntdst) + 1;

    float dotnrmlit2 = min(max(dot(Surface, pntvec), 0), 1);

    pntcol = dotnrmlit2 * DiffColorPt;
    pntcol = pntcol * pntdst;

    /* +term: directional diffuse light (sun) ---------------------------------- */
    float dotnrmlit1 = min(max(dot(Surface, LightVector), 0), 1);
    float4 envcol;

    envcol = dotnrmlit1 * DiffColor;
    envcol = (SunDimmer.x * envcol) + AmbientColor;

    /* +term: falloff ---------------------------------------------------------- */
    float envpower = pow(2.0, IN.blendindices.z);	// partial precision
    /* -term */

    OUT.ilum.rgb = (envcol.rgb * envpower) + pntcol.rgb;
    OUT.ilum.a = envcol.a;

    /* +term: fog -------------------------------------------------------------- */
    float fdensity = min(max((FogParam.x - length(InstanceProj.xyz)) / FogParam.y, 0), 1);

    OUT.fog.rgb = FogColor;
    OUT.fog.a = 1 - fdensity;

    /* +term: copy ------------------------------------------------------------- */
    OUT.texcoord_0.xy = IN.texcoord_0;

    return OUT;
};

// approximately 120 instruction slots used
