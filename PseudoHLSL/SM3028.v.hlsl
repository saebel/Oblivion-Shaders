...
#define	PI	3.14159274
    ...
    r0.w = (r0.w < -r0.w ? 1.0 : 0.0) * (max(IN.position.y, IN.position.x) == 0 ? 1.0 : 0.0);
    r0.z = (((r0.y * -2) + PI / 2) * (abs(IN.position.y) < abs(IN.position.x) ? 1.0 : 0.0)) + r0.y;
    r0.x = ((IN.position.y < -IN.position.y ? 1.0 : 0.0) * -PI) + r0.z;
    r2.yw = const_6.yw;
    OUT.texcoord_1.y = (IN.position.z * 0.025) + UOffset.x;
    OUT.texcoord_1.x = (((r0.w * -(2 * r0.x)) + r0.x) * r2.y) + VOffset.x;
