//
// Generated by Microsoft (R) D3DX9 Shader Compiler 9.08.299.0000
//
//   vsa shaderdump19/SLS2056.pso /Fcshaderdump19/SLS2056.pso.dis
//
//
// Parameters:
//
sampler2D NormalMap;
float4 PSLightColor[4];
sampler2D ShadowMap;
sampler2D ShadowMaskMap;
float4 Toggles;
//
//
// Registers:
//
//   Name          Reg   Size
//   ------------- ----- ----
//   PSLightColor[0]  const_2        1
//   Toggles       const_7       1
//   NormalMap     texture_1       1
//   ShadowMap     texture_2       1
//   ShadowMaskMap texture_3       1
//


// Structures:

struct VS_OUTPUT {
    float2 NormalUV : TEXCOORD0;			// partial precision
    float3 texcoord_1 : TEXCOORD1_centroid;
    float3 texcoord_2 : TEXCOORD2_centroid;
    float3 texcoord_3 : TEXCOORD3_centroid;			// partial precision
    float4 texcoord_4 : TEXCOORD4;			// partial precision
};

struct PS_OUTPUT {
    float4 color_0 : COLOR0;
};

// Code:

PS_OUTPUT main(VS_OUTPUT IN) {
    PS_OUTPUT OUT;

#define	expand(v)		(((v) - 0.5) / 0.5)
#define	compress(v)		(((v) * 0.5) + 0.5)
#define	shade(n, l)		max(dot(n, l), 0)
#define	shades(n, l)		saturate(dot(n, l))

    float1 q1;
    float3 q3;
    float1 q7;
    float4 r0;
    float3 t2;
    float1 t4;

    t4.x = tex2D(ShadowMaskMap, IN.texcoord_4.zw);			// partial precision
    t2.xyz = tex2D(ShadowMap, IN.texcoord_4.xy);			// partial precision
    r0.xyzw = tex2D(NormalMap, IN.NormalUV.xy);			// partial precision
    q7.x = r0.w * pow(abs(shades(normalize(expand(r0.xyz)), expand(IN.texcoord_3.xyz))), Toggles.z);			// partial precision
    q1.x = dot(normalize(expand(r0.xyz)), expand(IN.texcoord_2.xyz));			// partial precision
    q3.xyz = ((0.2 >= q1.x ? (q7.x * max(q1.x + 0.5, 0)) : q7.x) * PSLightColor[0].rgb) * IN.texcoord_1.xyz;
    OUT.color_0.a = 1;			// partial precision
    OUT.color_0.rgb = q3.xyz * ((t4.x * (t2.xyz - 1)) + 1);			// partial precision

    return OUT;
};

// approximately 32 instruction slots used (3 texture, 29 arithmetic)
