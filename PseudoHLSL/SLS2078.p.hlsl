//
// Generated by Microsoft (R) D3DX9 Shader Compiler 9.08.299.0000
//
//   psa shaderdump19/SLS2078.pso /Fcshaderdump19/SLS2078.pso.dis
//
//
// Parameters:

float4 PSLightColor[4];
sampler2D ShadowMap;


// Registers:
//
//   Name         Reg   Size
//   ------------ ----- ----
//   PSLightColor[0] const_2        1
//   ShadowMap    texture_6       1
//



// Structures:

struct VS_OUTPUT {
    float3 texcoord_6 : TEXCOORD6_centroid;
    float4 texcoord_1 : TEXCOORD1_centroid;			// partial precision
    float4 texcoord_2 : TEXCOORD2_centroid;
    float4 texcoord_3 : TEXCOORD3;			// partial precision
};

struct PS_OUTPUT {
    float4 color_0 : COLOR0;
};

// Code:

PS_OUTPUT main(VS_OUTPUT IN) {
    PS_OUTPUT OUT;

#define	PI	3.14159274
#define	D3DSINCOSCONST1	-1.55009923e-006, -2.17013894e-005, 0.00260416674, 0.00026041668
#define	D3DSINCOSCONST2	-0.020833334, -0.125, 1, 0.5

    const float4 const_0 = {0.5, 1, 15, 0};
    const float4 const_1 = {-0.000195312503, 0.01, -0.01, 0.25};
    const float4 const_3 = {1, -1, 0.6, 0};
    const float4 const_4 = {-0.01, 0.01, 0, 0};
    const int4 const_5 = {0, 1, 0, 0};

    float4 r0;
    float4 r1;
    float4 r2;
    float4 r3;
    float4 r4;
    float4 r5;

    r0.xy = (0.5 * ((1.0 / IN.texcoord_1.w) * IN.texcoord_1.xy)) + 0.5;	// [-1,+1] to [0,1]
    r4.x = saturate(r0.x >= 0.0 ? 0 : r0.x);
    r4.y = saturate((1 - r0.y) >= 0.0 ? 0 : (1 - r0.y));
    r0.xy = (const_3.xy * r0.xy) + const_5.xy;
    r5.w = saturate(length(IN.texcoord_6.xyz - IN.texcoord_2.xyz) / (IN.texcoord_2.w * 0.5));
    r1.w = sqrt(r5.w);
    r0.w = 1.0 / (((r1.w <= 0.0 ? (1 - r1.w) : 1)) * 15);			// partial precision
    r1.xyzw = tex2D(ShadowMap, saturate((r0.w * -const_4) + r0));
    r2.xyzw = tex2D(ShadowMap, saturate((r0.w * const_4) + r0));
    r3.xyzw = tex2D(ShadowMap, saturate((r0.w * 0.01) + r0));
    r0.xyzw = tex2D(ShadowMap, saturate((r0.w * -0.01) + r0));
    r4.xyzw = tex2D(ShadowMap, r4.xy);
    r0.w = ((-0.000195312503 - ((IN.texcoord_1.z / -IN.texcoord_2.w) + r4.x)) >= 0.0 ? 1 : 0);			// partial precision
    r0.w = r0.w + (((-0.000195312503 - ((IN.texcoord_1.z / -IN.texcoord_2.w) + r3.x)) >= 0.0 ? 1 : 0));			// partial precision
    r0.w = r0.w + (((-0.000195312503 - ((IN.texcoord_1.z / -IN.texcoord_2.w) + r2.x)) >= 0.0 ? 1 : 0));			// partial precision
    r0.w = r0.w + (((-0.000195312503 - ((IN.texcoord_1.z / -IN.texcoord_2.w) + r1.x)) >= 0.0 ? 1 : 0));			// partial precision
    r0.w = r0.w + (((-0.000195312503 - ((IN.texcoord_1.z / -IN.texcoord_2.w) + r0.x)) >= 0.0 ? 1 : 0));			// partial precision
    r0.w = (PSLightColor[0].a * (saturate(r0.w * 0.25) - 1)) + 1;			// partial precision
    r1.w = 1 - r0.w;			// partial precision
    r0.w = (r0.w <= 0.0 ? r1.w : 1);			// partial precision
    r0.xyz = lerp(1, lerp(1, (((r1.w * 0.6) * -PSLightColor[0].rgb) + 1), r5.w * r5.w), IN.texcoord_3.w);			// partial precision
    OUT.color_0.rgba = r0.xyzw;			// partial precision

    return OUT;
};

// approximately 66 instruction slots used (5 texture, 61 arithmetic)
