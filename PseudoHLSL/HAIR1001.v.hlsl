...
#define	sqr(v)			((v) * (v))
    ...
    eye18.xyz = mul(TanSpaceProj, normalize(normalize(EyePosition.xyz - IN.position.xyz) + LightDirection[0].xyz));
    lit3.xyz = LightPosition[1].xyz - IN.position.xyz;
    lit6.x = 1 - saturate(length(lit3.xyz) / LightPosition[1].w);
    OUT.color_0.a = 1;
    OUT.color_0.rgb = (shades(IN.normal.xyz, normalize(lit3.xyz)) * LightColor[1].rgb) * sqr(lit6.x);
