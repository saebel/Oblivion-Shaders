...
#define	sqr(v)			((v) * (v))
    ...
    r1.z = dot(normalize((q0.x * q35.xyz) + q34.xyz), LightDirection[0].xyz);
    ...
    m90.xy = mul(float2x4(ShadowProj[0].xyzw, ShadowProj[1].xyzw), r0.xyzw);
    q2.x = dot(ShadowProj[3].xyzw, r0.xyzw);
    q24.xyz = mul(float3x3(Bones[0 + offset.w].xyz, Bones[1 + offset.w].xyz, Bones[2 + offset.w].xyz), IN.tangent.xyz);
    q30.xyz = mul(float3x3(Bones[0 + offset.w].xyz, Bones[1 + offset.w].xyz, Bones[2 + offset.w].xyz), IN.binormal.xyz);
    q28.xyz = mul(float3x3(Bones[0 + offset.z].xyz, Bones[1 + offset.z].xyz, Bones[2 + offset.z].xyz), IN.binormal.xyz);
    q27.xyz = mul(float3x3(Bones[0 + offset.x].xyz, Bones[1 + offset.x].xyz, Bones[2 + offset.x].xyz), IN.binormal.xyz);
    q26.xyz = mul(float3x3(Bones[0 + offset.y].xyz, Bones[1 + offset.y].xyz, Bones[2 + offset.y].xyz), IN.binormal.xyz);
    r1.x = dot(normalize((q0.x * q24.xyz) + q21.xyz), LightDirection[0].xyz);
    ...
    r1.y = dot(normalize((q0.x * q30.xyz) + q29.xyz), LightDirection[0].xyz);
    OUT.texcoord_1.xyz = normalize(r1.xyz);
    OUT.texcoord_7.xy = ((q2.x * ShadowProjTransform.xy) + m90.xy) / (q2.x * ShadowProjTransform.w);
    OUT.texcoord_7.zw = ((m90.xy - ShadowProjData.xy) / ShadowProjData.w) * float2(1, -1) + float2(0, 1);
